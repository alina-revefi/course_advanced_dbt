...
- All non-derived boolean columns must have an accepted_values schema test. The accepted values are true and false.
- Downstream columns that contain category values must have an accepted_values schema test.
- Downstream columns that should never be null must have a not_null schema test.
- Downstream columns that should be unique must have a unique schema test.
